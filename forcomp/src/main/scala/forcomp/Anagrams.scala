...
object Anagrams extends AnagramsInterface:
   ...
   */
  def wordOccurrences(w: Word): Occurrences = (w.toLowerCase).toList.groupBy((x: Char)=>x).map((x,xs)=>(x,xs.length)).toSet.toList.sortWith((x1,x2)=>x1._1.compareTo(x2._1)<0)
  ...
  (
    (
      ...
      ).view.mapValues(_.map(_._2).sum).toMap.toList
    ) filter (_._2 != 0)
  ).sorted
