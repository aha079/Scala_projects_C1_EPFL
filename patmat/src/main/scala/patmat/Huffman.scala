...
trait Huffman extends HuffmanInterface:
  ...
  def weight(tree: CodeTree): Int = tree match 
    case Leaf(_,w) => w
    case Fork(l,r,_,w) => w // tree match ...

  def chars(tree: CodeTree): List[Char] = tree match 
    case Leaf(ch,_) => List(ch)
    case Fork(l,r,ch,_) => ch // tree match ...
   ...
   *   }
   */
  def times(chars: List[Char]): List[(Char, Int)] = if(chars.isEmpty) then Nil else getCharWeights(chars)
   ...
   */
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.sortWith((a, b) => a._2 < b._2).map((c) => Leaf(c._1, c._2))
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match 
    case l::r::tree => (makeCodeTree(l,r)::tree).sortWith((a,b)=>weight(a)<weight(b))
    case l::Nil => trees
    case Nil => trees
  ...
  private def encodeCharacter(tree: CodeTree, character: Char): List[Bit] = tree match {
    case Leaf(_,_) => Nil
    case Fork(left,right,_,_) => 
      if(chars(left) contains(character)) then 0::encodeCharacter(left, character)
